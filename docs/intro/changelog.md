...
### 更新内容：
- 新增分销商模式，覆盖申请准入、独立域名、专属站点配置、商品规则和订单归属，支持白标分销运营。
- 新增分销商财务与提现管理，支持分润入账、余额账户、流水明细、提现审核和退款扣回，账务处理更清晰。
- 新增用户侧分销商控制台，分销商可查看概览、订单、商品、域名、站点配置、账务流水和提现记录。
- 后台新增分销商运营管理入口，支持资料审核、域名管理、站点配置、商品规则、财务账户和提现处理。
- 支持站长选择 classic/vault 前台店面模板，并优化前台页面结构与组件复用，店面展示更灵活。
- 商品支持模糊库存展示，并修复 OKPay JSON 回调、网关币种、支付方式切换和部分商品对接问题，支付与下单流程更稳定。

## [v1.2.2] - 2026-06-14

### 更新内容：
- 新增 DujiaoPay 支付渠道，支持通过 DujiaoPay 接入收款，并优化支付回跳参数与回调处理。
- 新增批发价阶梯管理并统一到营销管理入口，前台可展示批发优惠，计费与兜底策略更一致。
- 新增注册邮箱域名白名单，支持限制可注册邮箱后缀，提升注册用户来源可信度。
- 新增卡密导出与素材批量删除能力，批量库存和素材维护更高效。
- 优化后台用户排序、活动价搜索、批发价筛选和列表展示，日常运营检索与管理更顺手。
- 修复 Telegram 用户等级分配、订单统计数量、默认语言展示和部分支付字段异常，前后台使用更稳定。
